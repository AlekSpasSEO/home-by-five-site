...
export function ProcessSteps({
  ...
  return (
    <ol
      className={`mt-12 border-t ${invert ? "border-bone/20" : "border-rule"}`}
    >
      {steps.map((step) => (
        <li
          key={step.n}
          className={`grid gap-4 border-b py-8 sm:grid-cols-[auto_1fr] sm:gap-10 ${
            invert ? "border-bone/20" : "border-rule"
          }`}
        >
          <span className="u-label u-tnum text-accent sm:w-16 sm:pt-1">
            {step.n}
          </span>
            ...
            <div>
              ...
              <p
                className={`mt-3 max-w-md text-[0.9375rem] leading-relaxed ${
                  invert ? "text-bone/65" : "text-ink-soft"
                }`}
              >
                {step.body}
              </p>
                ...
                {step.detail.map((d) => (
                  <li
                    ...
                    className={`text-[0.8125rem] ${
                      invert ? "text-bone/45" : "text-ink-mute"
                    }`}
                  >
                    {d}
                  </li>
        ...
        >
          <h3
            className={`u-display text-xl uppercase tracking-wide ${
              column.highlight || invert ? "text-bone" : "text-ink"
            }`}
          >
            {column.title}
          </h3>
          <p
            className={`mt-4 text-[0.9375rem] leading-relaxed ${
              column.highlight || invert ? "text-bone/70" : "text-ink-soft"
            }`}
          >
            {column.body}
          </p>
          {column.note ? (
            <p
              className={`u-label mt-5 ${
                column.highlight || invert ? "text-bone/40" : "text-ink-faint"
              }`}
            >
              {column.note}
            </p>
